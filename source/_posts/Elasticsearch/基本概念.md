...
## share（primary 数据分片) & replica(replica share数据副本)
- share快速的横向扩展，例如有4T的数据,此时可以把数据分配到4台es服务器上，可以有4个分类的type分布在4个share
- share 支持并发请求，以及高可用性
- 每一个replica也是一个share,用于数据备份
- 默认每个索引10个share,5个primary share 5个replica share,最小高可用是2台
